...
var errFileSpecDoesntMatchFormat = errors.New("文件格式必须是: [container:]file/path")
...
func extractFileSpec(arg string) (fileSpec, error) {
	...
	if i == -1 {
		return fileSpec{File: newLocalPath(arg)}, nil
	}
	containerName, file := arg[:i], arg[i+1:]
	if containerName == "" || file == "" || strings.Contains(containerName, "/") {
		return fileSpec{}, errFileSpecDoesntMatchFormat
	}
	return fileSpec{PodName: containerName, File: newRemotePath(file)}, nil
}
