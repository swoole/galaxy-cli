...
// 原库： https://github.com/modood/table
import (
	"errors"
	"fmt"
	"reflect"
...
func plain(coln []string, colw []int, rows [][]string) (table string) {
	if len(rows) == 0 {
		return ""
	}
	var head []rune
	for i, v := range colw {
		head = append(head, []rune(" "+coln[i]+repeat(v-StringLength([]rune(coln[i]))+1, ' '))...)
	}
	table += string(head) + "\n"
	...
	for _, r := range rows {
		...
		for i, v := range colw {
			...
			row = append(row, []rune(" "+r[i]+repeat(v-l+1, ' '))...)
		}
		body = append(body, row)
	}
